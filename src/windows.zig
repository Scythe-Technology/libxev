...
pub const exp = struct {
    ...
    pub const LPFN_CONNECTEX = *const fn (
        ...
        SockAddr: *const ws2_32.sockaddr,
    ...
    pub fn ReadFile(
        ...
        var read: DWORD = 0;
        const result = kernel32.ReadFile(handle, buffer.ptr, @as(DWORD, @intCast(buffer.len)), &read, overlapped);
        if (result == .FALSE) {
            const err = kernel32.GetLastError();
            return switch (err) {
                .IO_PENDING => null,
    ...
    pub fn WriteFile(
        ...
        var written: DWORD = 0;
        const result = kernel32.WriteFile(handle, buffer.ptr, @as(DWORD, @intCast(buffer.len)), &written, overlapped);
        if (result == .FALSE) {
            const err = kernel32.GetLastError();
            return switch (err) {
                .IO_PENDING => null,
