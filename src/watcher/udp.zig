                    ...
                    c.* = .{
                        ...
                        .callback = (struct {
                            fn callback(
                                ...
                                const addr = if (r.recvfrom) |_|
                                    net.Address.initPosix(@alignCast(&c_inner.op.recvfrom.addr)).toIpAddress()
                                else |_|
                                    net.Address.initIp4(.{ 127, 0, 0, 1 }, 0).toIpAddress();
                                return @call(.always_inline, cb, .{
                                    common.userdataValue(Userdata, s_inner.userdata),
                                    l_inner,
                                    c_inner,
...
fn UDPTests(comptime xev: type, comptime Impl: type) type {
    return struct {
        ...
        test "UDP: read/write" {
            ...
            var written: usize = 0;
            client.write(&loop, &c_write, &s_write, address, .{ .slice = &send_buf }, usize, &written, (struct {
                fn callback(
                    ud: ?*usize,
                    _: *xev.Loop,
                    _: *xev.Completion,
        ...
        test "UDP: read/write (2)" {
            ...
            const address = try std.Io.net.IpAddress.parse("127.0.0.1", 3132);
            const address2 = try std.Io.net.IpAddress.parse("127.0.0.1", 3133);
            const server = try Impl.init(address);
            const client = try Impl.init(address);
            ...
            var none_c_read: xev.Completion = undefined;
            var none_s_read: Impl.State = undefined;
            var none_recv_buf: [128]u8 = undefined;
            var none_recv_len: usize = 0;
            client.read(&loop, &none_c_read, &none_s_read, .{ .slice = &none_recv_buf }, usize, &none_recv_len, (struct {
                fn callback(
                    _: ?*usize,
                    _: *xev.Loop,
                    _: *xev.Completion,
